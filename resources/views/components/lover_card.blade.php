<form method="post" name="form{{ $order }}" action="/mypage/lovers/lover" class="lover_card">
    @csrf
        ...
        <a href="javascript:form{{ $order }}.submit()">
            <img @if($ext!=null) src="{{ asset('/storage/lover_imgs/'.sprintf('%09d', $lover->id).'.'.$ext.'?'.uniqid())}}?" @else src="/image/lover_icons/noimage.png" @endif alt="{{ $name }}" class="lover_card_img">
